...
double SpringForce::getRestLength0Value() {
	double pos1[DIM];
	double pos2[DIM];
	p1->GetPosition(pos1);
	p2->GetPosition(pos2);
...
void SpringForce::setColor(double red, double green, double blue, double alpha) {
	this->red = red;
	this->green = green;
	this->blue = blue;
	this->alpha = alpha;
}
GLdouble* SpringForce::getColors() {
	GLdouble colorMaterial[] = { this->red, this->green, this->blue, this->alpha };
	return &colorMaterial[4];
}
