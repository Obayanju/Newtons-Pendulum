...
public:
	...
	double getDx() { return pos[0]; }
	double getDy() { return pos[1]; }
	double getNextX() { return pos[0] + dir[0]; }
	double getNextY() { return pos[1] + dir[1]; }
	void setDx(double dx) { dir[0] = dx; }
	void setDy(double dy) { dir[1] = dy; }
	...
	void SwitchAnchored() { anchored = !anchored; }
...
public:
	...
	void SetDeltaT(double deltaT) { DeltaT = deltaT; }
	Particle * GetParticle(int i);

	void AddForce(Force * f);
	int GetNumForces();
	Force * GetForce(int i);
	vector<Particle*> GetParticles() { return pArray;  }
