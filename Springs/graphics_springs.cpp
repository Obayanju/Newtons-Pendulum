...
GLdouble redMaterial[] = {1.0, 0, 0, 1.0};
...
void checkForLeftCollision(int p1, std::vector<Particle*> const &All);
void checkForRightCollision(int p1, std::vector<Particle*> const &All);
void Collide(int p1, std::vector<Particle*>const &particles);
...
void display(void)
{
	...
	for(i=0; i<N; i++)
	{
		...
		checkForLeftCollision(1, PS.GetParticles());
		checkForRightCollision(N - 1, PS.GetParticles());
	}
	...
	for(i=0; i<NF; i++)
	{
		...
		if(f->Type() == SPRING_FORCE)
		{
			...
			GLdouble colorMaterial[] = { sf->getRed(), sf->getGreen(), sf->getBlue(), sf->getAlpha() };
			glColor3dv(colorMaterial);
			//glColor3dv(sf->getColors());
...
void checkForLeftCollision(int p1, std::vector<Particle*> const &All) {
	double x1 = All[p1]->getDx();
	double x2 = All[p1 - 1]->getDx();
...
void Collide(int p1, std::vector<Particle*>const &particles)
{
	int N = PS.GetNumParticles();
	if (p1 == 1) {
		particles[p1]->SwitchAnchored();
		particles[p1]->SetPositionx(particles[p1 - 1]->getDx());
		particles[p1 + (N - 2)]->SwitchAnchored();
		particles[p1 + (N - 2)]->setDx(2);
	}
	else if (p1 == N - 1) {
		particles[p1]->SwitchAnchored();
		particles[p1]->SetPositionx(particles[p1 - 1]->getDx());
		particles[1]->SwitchAnchored();
		particles[1]->setDx(-2);
	}
} /* Collide */
...
void InitParticles1()
{
	...
	std::string word;
	double x, y, xDir, yDir, r_temp, s_constant, d_constant, r_length, deltaG, red, green, blue, alpha;
	std::vector <double> reds, greens, blues, alphas;
	int p1, p2;
	bool anchored_temp;
	if (myFile.is_open()) {
		while (myFile >> word) {
			if (word == "p") {
				myFile >> x >> y >> xDir >> yDir >> r_temp >> anchored_temp;
				Particle *p = new Particle(x, y, xDir, yDir, r_temp, anchored_temp);
				PS.AddParticle(p);
			}
			else if (word == "s") {
				myFile >> p1 >> p2 >> s_constant >> d_constant >> r_length;
				if (r_length != 0) {
					// get rest_length 0 value
					SpringForce *sp = new SpringForce(PS.GetParticle(p1), PS.GetParticle(p2), s_constant, d_constant, r_length);
					double restLength0Value = sp->getRestLength0Value();
					r_length = restLength0Value * r_length;
				}
				Force *f = new SpringForce(PS.GetParticle(p1), PS.GetParticle(p2), s_constant, d_constant, r_length);
				PS.AddForce(f);
			}
			else if (word == "dg") {
				myFile >> deltaG;
				...
				for (int i = 0; i < NF; i++) {
					...
					if (f->Type() == SPRING_FORCE)
					{
						SpringForce * sf = (SpringForce*)f;
						if (i >= colorsSize)
							sf->setColor(reds[colorsSize-1], greens[colorsSize-1], blues[colorsSize-1], alphas[colorsSize-1]);
						else 
							sf->setColor(reds[i], greens[i], blues[i], alphas[i]);
					}
				}
			}
		}
